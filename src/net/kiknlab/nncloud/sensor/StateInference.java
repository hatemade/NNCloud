...
public class StateInference {//状態推定
	...
	// スレッド三回回ってわんわん
	final int INTERVAL_PERIOD = 500;// msecかと思ったか，あたりだよ！もう一本！
	...
	public StateInference(Context context) {
		...
		timer.schedule(new TimerTask() {
			...
			public void run() {
				while(timer!=null) {
					try {
						sp.edit().commit();// このあとはなるべくコミットしない
						inference();// 推定
					} catch (Exception e) {
					}
				}
	...
	private void inference() {//推定するよー
		...
		ArrayList<SensorData> orientations = LearningDBManager.getSensorData(
				mContext, SensorAdmin.TYPE_ORIENTATION_MAKE, java.lang.System.currentTimeMillis() - judgeTime);
	...
	public ArrayList<Float> calcVerticalAcceleration(ArrayList<SensorData> accele,ArrayList<SensorData> orientation){// 垂直加速度の計算
		ArrayList<Float> verticalAcceles = new ArrayList<Float>();
		int orientationIndex = 0;
		boolean checkIndex = false;
		Log.e("Debug Shiro","時間が増加してるかしえないか");
		for(int i = 0;i < accele.size();i++){
			while(checkIndex){//加速度の取得時と回転度の取得時がずれすぎていないかチェック、もっといい方法あるよなぁ、そのうち改善する…
				if(orientationIndex < (orientation.size() - 1))	orientationIndex++;
				else	checkIndex = true;
				if(orientation.get(orientationIndex).timestamp - accele.get(i).timestamp >
				orientation.get(orientationIndex-1).timestamp - accele.get(i).timestamp){
					orientationIndex--;
					checkIndex = true;
				}
			}
			verticalAcceles.add(calcVerticalAcceleration(accele.get(i), orientation.get(i)));
			checkIndex = false;
		}
		return verticalAcceles;
	}
	public float calcVerticalAcceleration(SensorData accele,SensorData orientation){// 垂直加速度の計算
